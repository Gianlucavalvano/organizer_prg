...
def _ensure_ore_schema(conn: Connection) -> None:
    with conn.cursor() as cur:
        cur.execute(
            """
...
@app.get("/ore-progetto/utente")
def ore_utente_corrente(
    user: AuthUser = Depends(get_current_user),
    conn: Connection = Depends(get_db_connection),
):
    _require_permission(user, PERM_APP_ORE_PROGETTO_OPEN)
    with conn.cursor() as cur:
        cur.execute("SELECT id_utente, username FROM utenti WHERE id_utente = %s LIMIT 1", (user.id_utente,))
        row = cur.fetchone()
    if row is None:
...
@app.get("/ore-progetto/progetti-attivi")
...
@app.get("/ore-progetto/mese/{mese}/chiuso")
def ore_mese_chiuso(
    mese: str,
    user: AuthUser = Depends(get_current_user),
    conn: Connection = Depends(get_db_connection),
    ...
    with conn.cursor() as cur:
        ...
        row = cur.fetchone()
    return {"mese": mese, "chiuso": bool(row and int(row[0] or 0) == 1)}


@app.post("/ore-progetto/mese/{mese}/chiudi")
def ore_chiudi_mese(
    mese: str,
    user: AuthUser = Depends(get_current_user),
    conn: Connection = Depends(get_db_connection),
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            """,
            (user.id_utente, mese, ts),
        )
    conn.commit()
    return {"ok": True, "mese": mese, "chiuso": True}


@app.post("/ore-progetto/mese/{mese}/riapri")
def ore_riapri_mese(
    mese: str,
    user: AuthUser = Depends(get_current_user),
    conn: Connection = Depends(get_db_connection),
...
@app.get("/ore-progetto/mese/{mese}/righe")
def ore_righe_mese(
    mese: str,
    user: AuthUser = Depends(get_current_user),
    conn: Connection = Depends(get_db_connection),
...
@app.get("/ore-progetto/mese/{mese}/totale")
def ore_totale_mese(
    mese: str,
    user: AuthUser = Depends(get_current_user),
    conn: Connection = Depends(get_db_connection),
...
@app.post("/ore-progetto/righe", status_code=status.HTTP_201_CREATED)
...
@app.delete("/ore-progetto/righe/{id_ore}")
...
def admin_moduli_utenti(
    ...
    with conn.cursor() as cur:
        ...
        rows = cur.fetchall()
    return [
        {"id_utente": int(r[0]), "username": r[1], "attivo": bool(r[2])}
        for r in rows
    ]
...
def admin_moduli_utente_put(
    ...
    with conn.cursor() as cur:
        ...
        missing = [c for c in codici if c not in by_code]
        if missing:
            raise HTTPException(
                status_code=400,
        ...
        for code, app_id in sorted(by_code.items(), key=lambda x: x[0]):
            cur.execute(
                ...
                VALUES (%s, %s, %s)
                """,
                (id_utente, app_id, code in codici),
            )

    conn.commit()
    return {"ok": True, "id_utente": id_utente, "moduli_effettivi": codici}


def _is_admin(user: AuthUser) -> bool:
    if (user.ruolo or "").upper() == "ADMIN":
        return True
    return "ADMIN" in [str(r).upper() for r in (user.ruoli or [])]
...
def _require_admin(user: AuthUser):
    if not _is_admin(user):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Permesso ADMIN richiesto",
        )


def _ensure_utenti_applicazioni_table(conn: Connection):
    with conn.cursor() as cur:
        cur.execute(
            """
